package me.whipmegrandma.grandmakits.api;
 ...
 */
public abstract class MenuEdit extends Menu {
	 ...
	 * @param startMode
	 */
	protected MenuEdit(Menu parent) {
		super(parent);
	...
	public final ItemStack getItemAt(int slot) {
		...
		if (slot > this.getSize() - 9 && slot != 44)
	...
	public final boolean isActionAllowed(final MenuClickLocation location, final int slot, final ItemStack clicked, final ItemStack cursor, final InventoryAction action) {
		...
		if (slot > this.getSize() - 10 && slot != 44)
			return false;
	...
	protected final void onMenuClose(Player player, Inventory inventory) {
		...
		for (int slot = 0; slot < 45; slot++)
			if (!new ArrayList<>(Arrays.asList(36, 37, 38, 39, 40, 41, 42, 43)).contains(slot))
